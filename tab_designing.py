...
def _guess_domain_from_upload(uploaded_file) -> str:
    ...
    if ext in {".mp4", ".mov", ".mkv", ".avi", ".webm", ".m4v"}:
        return "video"
    if ext in {".txt", ".md", ".csv", ".tsv", ".json", ".xml", ".html", ".py", ".yaml", ".yml", ".log"}:
        return "text"
    if ext in {".zip", ".gz", ".pdf", ".docx", ".pptx", ".xlsx"}:
    ...
    with tab_enc:
        ...
        with sub_comp:
            ...
            with left_col:
                ...
                with st.container(border=True):
                    st.markdown("### Initial Parameter")
                    mode_choice = st.selectbox("Option", ["Auto Fixed", "Auto Benchmark"], index=0, key="comp_mode")
